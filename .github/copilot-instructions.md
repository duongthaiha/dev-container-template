...
- The Microsoft Skills pack is **pre-installed into the image at build time** by a dedicated `RUN` step in `.devcontainer/Dockerfile` that runs (as the `vscode` user) a single `--full-depth --all` pass in a scratch dir, then relocates the tree to `/home/vscode/.copilot/skills/`:
  `npx --yes skills add microsoft/skills --full-depth --all --agent "*" -y` — every skill in `microsoft/skills`, including plugin skills under `.github/plugins/*/skills/` (e.g. microsoft-foundry, azure-deploy, azure-prepare, and 150+ others). Without `--full-depth` only the ~19 top-level skills under `.github/skills/` are discovered; the plugin skills are silently skipped.
