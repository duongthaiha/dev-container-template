...
- The Microsoft Skills pack is **pre-installed into the image at build time** by a dedicated `RUN` step in `.devcontainer/Dockerfile` that runs (as the `vscode` user) a single `--full-depth --all` pass in a scratch dir, then relocates the tree to `/home/vscode/.copilot/skills/`:
  `npx --yes skills add microsoft/skills --full-depth --all --agent "*" -y` — every skill in `microsoft/skills`, including plugin skills under `.github/plugins/*/skills/`. Without `--full-depth` only the ~19 top-level skills under `.github/skills/` are discovered; the plugin skills are silently skipped.
- After install, **non-Python language-specific skills** (suffixed `-dotnet`, `-java`, `-ts`, `-rust`) are pruned. This image targets Python workflows — language-agnostic skills (no suffix) and Python skills (`-py` suffix) are kept.
...
- This repo does **not** check in a local `.agents/skills/` tree or a `skills-lock.json`. Both are gitignored (see `.gitignore`). The image bake (`/home/vscode/.copilot/skills/`) is the single source of truth; a local `.agents/` checkout would drift from it and cause confusion. If you need to inspect skill contents for this repo's own development, run `npx skills add microsoft/skills --full-depth --all --agent "*" -y` in a scratch directory.
- Since `--full-depth --all` already installs every plugin skill, adding a new plugin skill from `microsoft/skills` requires no Dockerfile change — it will be picked up on the next weekly rebuild. To add a skill from a **different source**, append a separate pass: `&& npx --yes skills add <org>/<repo> --full-depth --all --agent "*" -y`. Do **not** move these into `postCreateCommand` — that would re-introduce the network-at-startup dependency we removed AND fail for consumers who keep their own `postCreateCommand`.
